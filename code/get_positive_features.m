...
for i = 1:num_images
    % read image
    image_path = strcat(train_path_pos, '/', image_files(i).name);
    img = im2single(imread(image_path));
    % create hog template
    imhog = vl_hog(img, feature_params.hog_cell_size);
    % reshape the size for easy usage
    features_pos(i, :) = reshape(imhog, 1, D);  
end
end
